...
The simplest thing is for you to enter a single column of values into Excel (include a column header with a name, no spaces). Then remember to save it in **.CSV** format into your **`Data`** sub-folder in your `MST2204` folder, e.g. `mynumbers.csv`. Then you simply need:
...
D = data.frame(
  value2 = c(rbeta(50,5,2), rbeta(50,2,5), rbeta(50,5,5)),
  group2 = rep(c('a', 'b', 'c'), each = N),
  ...
  group2_b = rep(c(0, 1, 0), each = N),
  group2_c = rep(c(0, 0, 1), each = N)
)
...
kruskal.test(value2 ~ group2, data = D)
